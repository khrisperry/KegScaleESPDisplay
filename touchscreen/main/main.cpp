...
bool scale_accepting_connection(uint8_t slot) {
  ...
  if (status != 200 || received <= 0) {
    if (slot == active_scale_index)
      touchscreen_ui_message(status == 404
  ...
  if (!o || !cJSON_IsBool(cJSON_GetObjectItemCaseSensitive(o, "paired"))) {
    ...
    cJSON_Delete(o);
    if (slot == active_scale_index)
      touchscreen_ui_message("Address did not return scale setup. Check the scale address.");
    return false;
  }
  ...
  if (slot == active_scale_index) {
    if (paired && !scale_paired(slot))
      touchscreen_ui_message("On scale: remove old Wi-Fi touchscreen, then Add touchscreen.");
    else if (!paired && scale_paired(slot))
      touchscreen_ui_message("In Setup: Remove pairing, then Add touchscreen on the scale.");
    else if (!ready)
      touchscreen_ui_message("On scale: Wi-Fi touchscreen setup > Add touchscreen.");
  }
  cJSON_Delete(o);
  return ready;
}
